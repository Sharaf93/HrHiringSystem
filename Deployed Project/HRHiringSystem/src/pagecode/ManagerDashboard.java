...
public class ManagerDashboard extends PageCodeBase implements Serializable{
	...
	private boolean employeeDropdownRender;
	...
	public void setEmployeesDropdown(){
		long currentUserID = Long.parseLong(FacesContext.getCurrentInstance().getExternalContext().getSessionMap().get("currentuserId").toString());
		String position = hrsourcesEJB.getPositionByID(currentUserID);
		String hrName = hrsourcesEJB.getHRNameByID(currentUserID);
		if(position.equals("HR Employee")){
			setChosenEmployee(hrName);
			setEmployeeDropdownRender(false);
		}else {
			setEmployeeDropdownRender(true);
		}
	}
	...
	public boolean isEmployeeDropdownRender() {
		return employeeDropdownRender;
	}

	public void setEmployeeDropdownRender(boolean employeeDropdownRender) {
		this.employeeDropdownRender = employeeDropdownRender;
	}

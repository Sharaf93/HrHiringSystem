...
public class Bean {
	...
	public List<Phas> getAllPhases ()
    {
    	...
    	Query query = em.createNamedQuery("getAllPhases1");
    ...
    public void persistTestsDetails ( List<TestsDetail> td )
    {
    ...
    public void mergeTestsDetails ( List<TestsDetail> td )
    {
    	for ( int i = 0; i < td.size() ; ++i)
    	{
    	em.merge(td.get(i));
    ...
    public void phasesDetailsListMerge( List<PhasesDetail> phDL)
    {
    	for(int i = 0 ; i < phDL.size() ; i++)
    	{
    		em.merge(phDL.get(i));
    	}
    ...
    public PhasesDetail getCertainPhaseDetail(Long candidateId, Long phaseId )
    {
    	...
    	System.out.println("candidate = " + candidateId + " phaseid = " + phaseId);
    	
    	return (PhasesDetail) query.getSingleResult();

    }
    
    public void persistNewPhaseDetail (PhasesDetail phD )
    {
    	em.persist(phD);
    }
    ...
    {
    	Query query = em.createNamedQuery("getTests");
